...
export const RegisterToken = () => {
  ...
  const request = async (token: string, device_id: string, platform: string) => {
    console.log('register token called with', token, device_id, platform);
    try {
      ...
      console.log('register token response', JSON.stringify(response, null, 2));
      if (response.success) {
